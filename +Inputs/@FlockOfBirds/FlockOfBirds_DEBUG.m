classdef FlockOfBirds < handle
    ...
    methods
        ...
        function [posAll, angAll] = getBirdGroup(obj)
            ...
            %% DEBUG
            clf
            hTriad(1) = PlotUtils.triad(eye(4),0.01);
            set(allchild(hTriad(1)),'color','r')
            hTriad(2) = PlotUtils.triad(eye(4),0.01);
            set(allchild(hTriad(2)),'color','g')
            hTriad(3) = PlotUtils.triad(eye(4),0.01);
            set(allchild(hTriad(3)),'color','b')
            hTriad(4) = PlotUtils.triad(eye(4),0.01);
            set(allchild(hTriad(4)),'color','m')
            axis equal
            axis([-.1 .3 -.1 .1 -.1 .1])
            for i = 1:7:numValid
                ...
                F = makehgtform('translate',pos,... % convert to 4x4 frame
                    'zrotate',ang(1),'yrotate',ang(2),'xrotate',ang(3));

                set(hTriad(msg(7,i)-1),'Matrix',F)
                drawnow
            end
