...
class Deserialise {
    ...
    classifier_event_t(Buffer){
        ...
        if(Buffer.readUInt8(0) == 3){
            ...
            // arm synced  TODO: Not tested
        } else if (Buffer.readUInt8(0) == 1){
            return {
                type: Buffer.readUInt8(0),
                arm: Buffer.readUInt8(1),
                x_direction: Buffer.readUInt8(2)
            };
            // syc failed TODO: Not tested
        } else if(Buffer.readUInt8(0) == 6){
            return {
                type: Buffer.readUInt8(0),
                sync_result: Buffer.readUInt8(1)
            };
        }
        return {
            type: Buffer.readUInt8(0),
