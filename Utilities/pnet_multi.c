...
double my_now(){
    ...
    sec=mxGetScalar(plhs[0])*60*60*24;
    dotimenow=floor(sec*10)/10; 
    mxDestroyArray(plhs[0]);
    if(lastdotime!=dotimenow){  
...
void my_mexReturnArrayFromBuff(const int argno,io_buff *buff,const int line)
{
	...
	for(n=0;n<returnelements;n++)
		...
		break;
	if(n==maxelements)                            
	    deleteelements=returnelements=maxelements;
	else if(n==returnelements)                     
	    deleteelements=returnelements=0;          
	else if(n>0 && buff->ptr[n-1]=='\r')          
...
/*  //   mexPrintf("DEBUG MEX(2):[%d] %s\n",con_index,fun);    DEBUG  */
	...
	gprhs=(const mxArray **)plhs; gnrhs=1; 
