    ...
    methods
        ...
        function update_sensory(obj)
            ...
            if ~isempty(obj.TactorPort)
                ...
                %Thumb is weak, need to treat separately
                minSensorThumb = .1;
                sensorVals(1) = sensorVals(1);
                sensorVals(1) = max(sensorVals(1),minSensorThumb);
                sensorVals(1)
                
                % scale the torque values and perform mapping
                tactorVals(5) = interp1([minSensorThumb, 0.3],[0 255],sensorVals(1),'linear',255);
